...
df2 = pd.read_csv('./models/recommender/final_extended_dataset2.csv')
makeup = pd.read_csv('./models/recommender/makeup_dataset2.csv')
entries = len(df2)
LABELS = list(df2.label.unique())
# features


features = ['normal', 'combination', 'dry', 'oily', 'dullness', 'loss of firmness', 'elasticity', 'fine lines', 'wrinkles', 'pores', 'redness', 'uneven texture', 'dark spots', 'oiliness', 'acne', 'blemishes', 'dryness.', 'dark circles', 'puffiness']
...
def wrap(info_arr):
    ...
    result = {'brand': info_arr[0], 'name': info_arr[1], 'price': info_arr[2], 'url': info_arr[3],
              'skin_type': info_arr[4], 'concern': str(info_arr[5]).split(','), 'img': info_arr[6]}
#     print(info_arr)
    return result

def wrap_makeup(info_arr):
    result = {'brand': info_arr[0], 'name': info_arr[1], 'price': info_arr[2], 'url': info_arr[3], 'img': info_arr[4],
              'skin_type': info_arr[5], 'skin_tone': info_arr[6]}
#     print(info_arr)
    return result
...
def makeup_recommendation(skin_tone):
    ...
    data = dff[['brand', 'name', 'price', 'url', 'img', 'skin_type', 'skin_tone']].to_dict('split')['data']
    for element in data:
